...
function hasDirective(node: any, name: string): boolean {
	return (node.props || []).some((prop: any) => prop.type === NODE_TYPE_DIRECTIVE && prop.name === name);
}
...
function isConditionalChain(elements: any[]): boolean {
	...
	return elements.slice(1).every(el => hasDirective(el, 'else-if') || hasDirective(el, 'else'));
}
...
export const viteCheckTransitionPlugin = (): PluginOption => {
	...
	return {
		...
		transform(code, id) {
			...
			if (normalizedId.includes('?') || !normalizedId.endsWith('.vue') || !normalizedId.includes(targetDir)) {
				return null;
			}
			...
			const isChain = rootNodes.length === rootElements.length && isConditionalChain(rootElements);

			if (rootNodes.length > 1 && !isChain) {
				const relativePath = normalizedId.slice(normalizedId.indexOf('/src/') + 1);
