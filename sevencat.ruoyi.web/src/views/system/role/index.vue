	...
	<div class="p-2 system-role-page">
		...
		<el-dialog v-model="openDataScope" :title="dialog.title" width="760px" append-to-body>
			<el-form
				...
				:model="form"
				label-width="90px"
				class="dialog-grid-form permission-dialog-form"
			>
								...
								<template #default="{ data, node }">
									<div class="menu-tree-node-row">
										<span
											...
											:class="{
												'is-hidden': isMenuPermissionHidden(data),
										...
										>
											...
											<el-tooltip
												v-if="isMenuPermissionHidden(data)"
												content="隐藏"
												placement="top"
											>
										...
										<div
											v-if="data.buttonPermissions?.length"
											...
											>
												...
												>
													...
													<el-tooltip v-if="button.disabled" content="停用" placement="top">
														<el-icon class="menu-button-disabled-icon">
															<CircleCloseFilled />
														</el-icon>
													</el-tooltip>
												</span>
...
const buildCheckedButtonPermissionIds = (
	...
	if (form.value.menuCheckStrictly) {
		newlyCheckedMenuIds.forEach(menuId => {
			menuPermissionMeta.value.menuButtonIdsMap
				.get(String(menuId))
				?.forEach(buttonId => nextButtonIds.add(buttonId));
		});
	}
